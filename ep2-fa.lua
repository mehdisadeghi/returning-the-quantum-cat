	...
	act = function(s, w)
		...
		if w == 1 then
			if not have('food') then
				return 'دور یه میز خالی نشستم. خب، کمی استراحت کردم - حالا وقت رفتنه!';
			end
			return walk('eating');
		end
		if w == 2 then
...
wside = room {
	...
	act = function(s, w)
		if w == 1 then
			return 'ونی که قصه من از اونجا شروع شد...';
		end
		if w == 5 then
...
eroom = room {
	...
	enter = function(s, f)
		...
		if f == 'toilet' then
			return 'خب... توری فلزی کف توالت رو بلند می‌کنم و وارد تاریکی می‌شم... ظرف چند دقیقه از سوراخ تهویه هوا می‌پرم روی میز و میام روی زمین.';
		end
	end,
	act = function(s, w)
		if w == 1 then
...
facectrl = dlg {
	...
	obj = {
		...
		'— برام مهم نیست! نگاهی به خودت بنداز! اواخر اصلا یه آینه دیدی؟ اومدی به خود بلین گوش بدی! بـ-لـین! دست راست... - مامور لحظه‌ای با احترام تامل می‌کنه - خب... بزن به چاک!',
		[[pon(3,4)]]),
